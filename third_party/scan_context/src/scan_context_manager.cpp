...
ScanContextManager::ScanContextManager(const YAML::Node & node)
{
  ...
  std::cout << "Scan Context params:" << std::endl
            ...
            << "\tloop-closure scan context distance thresh: " << SCAN_CONTEXT_DISTANCE_THRESH_
            << std::endl
            << std::endl;
...
bool ScanContextManager::save(const std::string & output_path)
{
  ...
  if (UpdateIndex(0)) {
    ...
    if (!SaveIndex(index_output_path)) {
      std::cerr << "[Scan Context]: Failed to write index." << std::endl;
      return false;
    } else {
      std::cout << "\tSave index to: " << index_output_path << std::endl;
    }
    ...
    if (!SaveScanContexts(scan_contexts_output_path)) {
      std::cerr << "[Scan Context]: Failed to write scan contexts." << std::endl;
      return false;
    } else {
      std::cout << "\tSave scan context of size " << state_.scan_context_.size()
    ...
    if (!SaveRingKeys(ring_keys_output_path)) {
      std::cerr << "[Scan Context]: Failed to write ring keys." << std::endl;
      return false;
    } else {
      std::cout << "\tSave ring keys of size " << state_.index_.data_.ring_key_.size()
    ...
    if (!SaveKeyFrames(key_frames_output_path)) {
      std::cerr << "[Scan Context]: Failed to write key frames." << std::endl;
      return false;
    } else {
      std::cout << "\tSave key frames of size " << state_.index_.data_.key_frame_.size()
                << " to: " << key_frames_output_path << std::endl;
    }

    google::protobuf::ShutdownProtobufLibrary();
  } else {
    std::cerr << std::endl << "[Scan Context]: Skip empty index" << std::endl << std::endl;
  }
...
bool ScanContextManager::load(const std::string & input_path)
{
  ...
  std::cout << std::endl << "[Scan Context]: Load Scan Context Index." << std::endl;

  // 1. scan contexts:
  std::string scan_contexts_input_path = input_path + "/scan_contexts.proto";
  if (!LoadScanContexts(scan_contexts_input_path)) {
    std::cerr << "[Scan Context]: Failed to load scan contexts." << std::endl;
    return false;
  } else {
    std::cout << "\tNum. Scan Contexts: " << state_.scan_context_.size() << std::endl;
  }

  // 2. ring keys:
  std::string ring_keys_input_path = input_path + "/ring_keys.proto";
  if (!LoadRingKeys(ring_keys_input_path)) {
    std::cerr << "[Scan Context]: Failed to load ring keys." << std::endl;
    return false;
  } else {
    std::cout << "\tNum. Ring Keys: " << state_.ring_key_.size() << std::endl;
  }

  // 3. key frames:
  std::string key_frames_input_path = input_path + "/key_frames.proto";
  if (!LoadKeyFrames(key_frames_input_path)) {
    std::cerr << "[Scan Context]: Failed to load key frames." << std::endl;
    return false;
  } else {
    std::cout << "\tNum. Key Frames: " << state_.index_.data_.key_frame_.size() << std::endl;
  }
  ...
  std::cout << "\tIndex Size: " << state_.index_.kd_tree_->kdtree_get_point_count() << std::endl;
 ...
 */
ScanContextManager::RingKey ScanContextManager::getRingKey(const ScanContext & scan_context)
{
...
void ScanContextManager::generateRandomRingKey(
  ScanContextManager::RingKeys & samples, const int N, const int D, const float max_range)
{
  samples.clear();
  samples.resize(N);
  for (int i = 0; i < N; ++i) {
 ...
 */
Eigen::MatrixXf ScanContextManager::GetSectorKey(const Eigen::MatrixXf & scan_context)
{
 ...
 */
int ScanContextManager::GetIndex(const float & value, const float & MAX_VALUE, const int RESOLUTION)
{
...
{
  state_.index_.kd_tree_->query(&ring_key.at(0), N, &indices.at(0), &distances.at(0));
}
...
{
  const int N = targetScanContext.cols();

  int num_effective_cols = 0;
  float sum_sector_similarity = 0.0f;
  for (int sid = 0; sid < N; ++sid) {
    const Eigen::VectorXf target_sector = targetScanContext.col(sid);
    ...
    float sector_similarity =
      target_sector.dot(source_sector) / (target_sector_norm * source_sector_norm);
  ...
  const ScanContext & targetScanContext, const ScanContext & source_scan_context)
{
  ...
  Eigen::MatrixXf target_sector_key = GetSectorKey(targetScanContext);
  Eigen::MatrixXf source_sector_key = GetSectorKey(source_scan_context);
  int sector_key_shift = GetOptimalShiftUsingSectorKey(target_sector_key, source_sector_key);
  ...
  for (int curr_shift : candidate_shifts) {
    ScanContext targetScanContext_shifted = CircularShift(targetScanContext, curr_shift);

    float curr_dist = GetCosineDistance(targetScanContext_shifted, source_scan_context);
...
{
  if (state_.ring_key_.size() > static_cast<size_t>(MIN_KEY_FRAME_SEQ_DISTANCE)) {
  ...
  const ScanContext & query_scan_context, const RingKey & query_ring_key)
{
  ...
  GetCandidateIndices(query_ring_key, NUM_CANDIDATES_, candidate_indices, candidate_distances);
  ...
  for (int i = 0; i < NUM_CANDIDATES_; ++i) {
    const ScanContext & candidate_scan_context = state_.scan_context_.at(candidate_indices.at(i));
...
{
  std::ofstream output_f(output_path, std::ofstream::binary);
  if (output_f.bad()) {
    return false;
  }

  state_.index_.kd_tree_->index->saveIndex(output_f);
...
bool ScanContextManager::LoadScanContexts(const std::string & input_path)
{
  ...
  for (int i = 0; i < scan_contexts.data_size(); ++i) {
    ...
    ScanContext output_scan_context =
      ScanContext::Zero(scan_contexts.num_rings(), scan_contexts.num_sectors());
