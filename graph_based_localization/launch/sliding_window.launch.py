...
def generate_launch_description():
    ...
    pkg_graph_based_localization = get_package_share_directory(
        "graph_based_localization"
    )
    rviz2_config = os.path.join(
        pkg_graph_based_localization, "launch", "sliding_window.rviz"
    )
    ...
    sliding_window_node = Node(
        ...
        package="graph_based_localization",
        executable="sliding_window_node",
        parameters=[{"config_file": sliding_window_config}],
        output="screen",
    )
